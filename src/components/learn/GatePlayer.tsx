/**
 * GatePlayer — The gate phase: final assessment before level completion.
 *
 * No hints shown. Must score >= 90% to pass.
 */

import { useState, useCallback } from 'react';
import { Check, ChevronRight, Shield, AlertTriangle } from 'lucide-react';
...
export default function GatePlayer({
  drills,
  levelColor,
  levelId,
  onComplete,
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h3 className="text-sm font-bold text-[#eff1f6]">Gate Check</h3>
          <p className="text-[11px] text-[#8c8c8c]">
            Need {required}/{drills.length} to pass
          </p>
        </div>
        <div
          className="text-xs font-bold px-2.5 py-1 rounded-md"
          style={{
            color: score >= required ? '#22c55e' : levelColor,
            backgroundColor: score >= required ? '#22c55e15' : `${levelColor}15`,
          }}
        >
          {score}/{drills.length}
        </div>
      ...
      <div>
        <div className="flex justify-between text-[10px] text-[#8c8c8c] mb-1.5">
          <span className="font-medium">
            Question {index + 1} of {drills.length}
          </span>
      ...
      {/* No hints warning */}
      <div className="flex items-center gap-2 rounded-xl bg-[#ef444410] border border-[#ef444430] p-3">
        <AlertTriangle size={14} className="text-[#ef4444] flex-shrink-0" />
        <p className="text-xs text-[#ef4444]">
          No hints in the gate. Answer from memory.
        </p>
      </div>
      ...
      <div className="rounded-xl bg-[#141414] border border-[#ffffff08] p-4 space-y-4">
        ...
        {drill.type === 'word-scramble' && drill.scrambledWords && (
